...
const Shell = ({ children, isLoggedIn, mobileMenuOpen, setMobileMenuOpen }: ShellProps) => (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasOpenings) {
      if (openings.length === 0) {
        setError("Please specify at least one job opening.");
        return;
      }
      for (let i = 0; i < openings.length; i++) {
        const op = openings[i];
        if (!op.role.trim()) {
          setError(`Please specify the role title for opening #${i + 1}.`);
          return;
        }
        if (op.count < 1) {
          setError(`Please specify a valid count (min 1) for opening #${i + 1}.`);
          return;
        }
        if ((op.mode === "On-site" || op.mode === "Hybrid") && !op.location.trim()) {
          setError(`Please specify the location for opening #${i + 1} (${op.mode}).`);
          return;
        }
      }
    }
      ...
      <div className="max-w-2xl mx-auto py-8 sm:py-12 px-3 sm:px-6" style={{ fontFamily: "'Sora', 'Segoe UI', Arial, sans-serif" }}>
        <div className="bg-white border border-[#E7E2D8] rounded-2xl shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="px-4 sm:px-8 py-6 sm:py-7 flex flex-col gap-6">
            ...
            {/* Has Openings Toggle */}
            <div className="flex flex-col gap-3 p-4 border border-[#E7E2D8] bg-[#FDFBF7] rounded-xl">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-[#000666]">Internship / Job Openings</h4>
                  <p className="text-xs text-slate-500 mt-0.5">Do you have specific roles you'd like our participants to fill?</p>
                </div>
              ...
              {hasOpenings && (
                <div className="flex flex-col gap-4 mt-2 border-t border-[#E7E2D8]/60 pt-4">
                  {openings.map((op, index) => (
                    <div key={index} className="flex flex-col gap-3 p-4 border border-[#E7E2D8] bg-white rounded-xl relative">
                      {openings.length > 1 && (
                        <button
                          type="button"
                          onClick={() => removeOpening(index)}
                          className="absolute top-3 right-3 text-red-500 hover:text-red-700 text-xs font-bold transition-colors cursor-pointer"
                        >
                          Remove
                        </button>
                      )}
                      <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Opening #{index + 1}</h5>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                          <label className="text-xs font-bold text-slate-600">Number of Roles *</label>
                          <input
                            type="number"
                            min={1}
                            required
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                          ...
                          <select
                            ...
                            className="w-full px-3 py-2 border border-[#E7E2D8] bg-white rounded-lg text-sm focus:outline-none focus:border-[#000666]"
                          >
                  ...
                  <button
                    type="button"
                    onClick={addOpening}
                    className="self-start text-xs font-bold text-[#000666] hover:text-[#000666]/80 flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    + Add another opening
                  </button>
                </div>
