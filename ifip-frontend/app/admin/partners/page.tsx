...
export default function AdminPartnersPage() {
  ...
  const handleOpenCreate = () => {
    ...
    setContactEmail(""); setContactPerson(""); setContactPhone("");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      showToast("success", "Partner organization deleted successfully.");
      fetchPartners();
    } catch (err) {
      console.error("Failed to delete partner:", err);
      showToast("error", "Failed to delete partner organization.");
    }
  };

  const handleSendInvite = async (partner: PartnerOrg) => {
    if (!partner.contactEmail) {
      showToast("error", "This partner has no contact email set. Edit the org to add one first.");
      return;
    }
    setInvitingId(partner._id);
    try {
      await sendPartnerInvite(partner._id);
      showToast("success", `Portal invite sent to ${partner.contactEmail}`);
      fetchPartners();
    } catch (err: any) {
      console.error("Failed to send partner invite:", err);
      showToast("error", err?.response?.data?.message || "Failed to send partner portal invite.");
    } finally {
      setInvitingId(null);
    }
  };
  ...
  const handleOrgSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingPartner) {
        ...
        showToast("success", "Partner organization updated successfully.");
      } else {
        await createPartnerV2(payload);
        showToast("success", "New partner organization registered successfully.");
      }
      setModalOpen(false);
      fetchPartners();
    } catch (err) {
      console.error("Failed to save partner details:", err);
      showToast("error", "Failed to save partner organization details.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleReview = async (action: "approve" | "decline") => {
    ...
    try {
      ...
      showToast(
        "success",
        action === "approve"
          ? `${reviewingApp.companyName} approved and added as an active partner.`
      ...
      showToast("error", err?.response?.data?.message || "Review action failed. Please try again.");
    } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4 sm:px-6 lg:px-8 font-sans bg-[#FDFBF7]">
      {/* Toast notification */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-[9999] flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl border text-sm font-semibold animate-fadeIn ${
          toast.type === "success"
                  ...
                  <div className="border-t border-slate-100 pt-4 mt-3">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      {partner.portalEnabled ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
                          <HiOutlineCheckCircle className="w-3 h-3" /> Portal Active
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500 border border-slate-200">
                          <HiOutlineClock className="w-3 h-3" /> No Portal Access
                        </span>
                      )}
                      {partner.inviteSentAt && (
                        <span className="text-[10px] text-slate-400 font-medium">
                          Last invite: {new Date(partner.inviteSentAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
                        </span>
                      )}
                    </div>
                    {/* Contact email row */}
                    {partner.contactEmail && (
                      <div className="flex items-center gap-1.5 mb-3">
                        <HiOutlineEnvelope className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span className="text-[11px] text-slate-500 font-medium truncate">{partner.contactEmail}</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-slate-500 font-medium">
                        Slots: <strong className="text-[#000666]">{partner.activeSlots}</strong>
                      </div>
                      <div className="flex gap-2">
                        <button
                          ...
                          title={partner.portalEnabled ? "Resend Portal Invite" : "Send Portal Invite"}
                        >
                          ...
                          {partner.portalEnabled ? "Resend Invite" : "Send Invite"}
                        </button>
                        <button onClick={() => handleOpenEdit(partner)}
                          className="p-1.5 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 hover:text-emerald-600 transition-colors cursor-pointer" title="Edit Partner">
                          <HiOutlinePencilSquare className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(partner._id)}
                          className="p-1.5 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 hover:text-red-600 transition-colors cursor-pointer" title="Delete Partner">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-emerald-50/50 border border-emerald-100 rounded-xl">
                <div className="sm:col-span-2">
                  <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    <HiOutlineEnvelope className="w-3.5 h-3.5" /> Portal Invite Contact
                  </p>
                </div>
                <div className="flex flex-col gap-1.5">
